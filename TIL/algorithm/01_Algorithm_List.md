...
- 값의 양이 길어질수록 효율이 극악으로 떨어진다

- 구현이 간단 

- **최소 O(n), 최대 O(n^2) 시간복잡도를 갖는다**

```python
for i in range(1, len(arr)): # 리스트의 크기만큼 반복
    for j in range(i, 0, -1): # j 인덱스의 값이 줄어드면서 삽입할 위치를 찾을 때까지 반복
        if arr[j] < arr[j-1]: # 현재 인덱스가 앞의 원소보다 작다면
            arr[j], arr[j-1] = arr[j-1], arr[j] # swap해서 값 뒤로 밀어내기

print(arr)# 정렬 후 리스트 출력
```
